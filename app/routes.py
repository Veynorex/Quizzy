...
def create_db():
    ...
    if not admin:
        admin = User(username = os.getenv('ADMIN_USERNAME'), fullname = "Admin Huseyn")
...
def admin_login():
    ...
    if form.validate_on_submit():
        ...
        if user and user.username == form.username.data and user.check_password(form.password.data):
            login_user(user)
            flash("Admin logged in succesfully", category = "success")
...
def edit_subject(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("Subject updated succesfully!", category="success")
        return redirect(url_for("manage_subjects"))
...
def delete_subject(id):
    ...
    db.session.commit()
    flash(" Subject deleted succesfully!", category="success")
...
def edit_chapter(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(" Chapter updated succesfully!", category="success")
        return redirect(url_for("manage_chapters"))
...
def delete_chapter(id):
    ...
    db.session.commit()
    flash(" Chapter deleted succesfully!", category="success")
...
def edit_quiz(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(" Quiz updated succesfully!", category="success")
        return redirect(url_for("manage_quizzes"))
...
def delete_quiz(id):
    ...
    db.session.commit()
    flash(" Quiz deleted succesfully!", category="success")
    return redirect(url_for("manage_quizzes"))
...
def add_question(quiz_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("The question entered succesfully!", category="success")
...
def register():
    form = RegisterForm()
    if form.validate_on_submit():
        user = User(username = form.username.data, fullname = form.fullname.data, qualification = form.qualification.data, dob = form.dob.data)
        user.set_password(form.password.data)
        db.session.add(user)
        db.session.commit()
        flash("Registration was succesfull", category="success")
        return redirect(url_for('login'))
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and user.check_password(form.password.data):
            login_user(user)
            flash("You logged in succesfully", category="success")
...
def attempt_quiz(quiz_id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        flash(f'Your score: {score} / {len(questions)}', category="success")
